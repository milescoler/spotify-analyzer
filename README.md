...
- **Add Words (authoring).** Teachers just name a subject and paste a vocab
  list — one word per line, an optional definition after a dash or colon. The
  AI writes any missing definitions, examples, and the interest-personalized
  practice scenarios; the teacher never has to author questions.

## AI content (optional API key)

The actual learning content — definitions/examples for words a teacher didn't
define, and the interest-themed practice scenarios — is generated by **Claude**.
The app is **key-ready but runs without one**: with no key it stores clearly
labeled *sample* content (flagged so it can be auto-filled later).

To enable real AI content, drop your key in a `.env` file (see `.env.example`):
...
The server loads `.env` automatically (no extra tooling). Defaults to
`claude-opus-4-8`; set `WORD_GARDEN_MODEL` for a faster/cheaper tier on a big
class. Uses the Messages API over `fetch` to keep the zero-install footprint.
