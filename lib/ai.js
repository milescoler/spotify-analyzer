...
// ---- word definitions (teacher gives the word; AI fills meaning + example) --

const DEFINE_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  properties: { definition: { type: 'string' }, example: { type: 'string' } },
  required: ['definition', 'example'],
};

// Produce a kid-friendly definition + example for a word. The teacher MAY supply
// a definition; if so we keep it and only need an example. With an API key set,
// Claude writes the content; otherwise we store a clearly-labeled sample so the
// app works now and can be auto-filled later (see `aiPending`).
export async function defineWord({ word, subject, grade = 3, definition }) {
  const teacherDef = definition && definition.trim() ? definition.trim() : null;
  if (aiEnabled()) {
    try {
      const r = await defineWithClaude({ word, subject, grade, definition: teacherDef });
      return { definition: r.definition, example: r.example, aiGenerated: !teacherDef, aiPending: false };
    } catch (err) {
      console.error('defineWord via Claude failed, using sample:', err.message);
    }
  }
  return {
    definition: teacherDef || `(Sample) “${word}” — a vocabulary word in ${subject}. Add an API key to auto-generate the real meaning.`,
    example: `Try using the word “${word}” in a sentence about ${subject}.`,
    aiGenerated: false,
    aiPending: !teacherDef, // a real definition is still owed once a key is added
  };
}

async function defineWithClaude({ word, subject, grade, definition }) {
  const system =
    `You write kid-friendly vocabulary content for ${grade}-grade students. ` +
    `Definitions are one short, simple sentence a ${grade}-grader can read. ` +
    `Examples are one friendly sentence that uses the word naturally.`;
  const user = definition
    ? `Word: "${word}" (subject: ${subject}). Keep this teacher definition exactly: ` +
      `"${definition}". Write one simple example sentence. Return JSON {definition, example}.`
    : `Define the word "${word}" for a ${grade}-grade student studying ${subject}. ` +
      `Return JSON with a simple "definition" and one "example" sentence using the word.`;
  return callJSON(system, user, DEFINE_SCHEMA);
}
...
async function generateWithClaude({ word, definition, example, interest, grade }) {
  ...
  const scenario = await callJSON(system, user, SCENARIO_SCHEMA);
  return sanitize(scenario);
}

// One Claude Messages API call that returns JSON matching `schema`.
